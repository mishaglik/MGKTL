...
namespace mgk {
    ...
    void OTextStream::putString(const char* string, size_t len)
    {
        if(!buffer_)
        {
            write(string, len);
    ...
    void OTextStream::putChar(char c)
    {
        if(!buffer_)
        {
            write(&c, 1);
    ...
    OTextStream& OTextStream::operator<<(bool cond)              
    {
        putString(cond ? "true" : "false", cond ? 4 : 5);
        return *this;
    }

    OTextStream& OTextStream::operator<<(char c)              
    {
        putChar(c);
        return *this;
    }

    OTextStream& OTextStream::operator<<(uint32_t x)          
    {
        putNumber(x, NumFlags::None);
        return *this;
    }

    OTextStream& OTextStream::operator<<(int32_t x)           
    {
        putNumber(x, NumFlags::Signed);
        return *this;
    }

    OTextStream& OTextStream::operator<<(int64_t x)           
    {   
        putNumber(x, NumFlags::Signed);
        return *this;
    }

    OTextStream& OTextStream::operator<<(uint64_t x)          
    {
        putNumber(x, NumFlags::None);
        return *this;
    }

    OTextStream& OTextStream::operator<<(format::hex x)          
    {
        putNumber(x.x, NumFlags::Hex);
        return *this;
    }

    OTextStream& OTextStream::operator<<(format::oct x)          
    {
    ...
    OTextStream& OTextStream::operator<<(std::nullptr_t)    
    {
        putString("nullptr", sizeof("nullptr") - 1);
        return *this;
    }

    OTextStream& OTextStream::operator<<(const char* s)       
    {
        size_t len = strlen(s);
        putString(s, len);
        return *this;
    ...
    OTextStream& OTextStream::operator<<(const void* s)       
    {
        putNumber(reinterpret_cast<uint64_t>(s), NumFlags::Ptr);
        return *this;
    }

    OTextStream& OTextStream::operator<<(const std::string& s)
    {
        putString(s.c_str(), s.length());   
        return *this;
    }

    OTextStream& OTextStream::operator<<(endl_t)              
    {
        putChar('\n');
        flush();
        return *this;
    ...
    void OTextStream::putNumber(uint64_t number, uint64_t numFlags)
    {
